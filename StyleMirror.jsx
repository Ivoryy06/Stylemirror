  ...
  return (
    <Card style={{ marginTop:"1rem" }}>
      ...
      {data ? (
        ...
        <p style={{ fontSize:13, color:"#888780", fontFamily:FONTS.ui }}>Check how original your continuation is vs your saved samples.</p>
      )}
    </Card>
  );
};
...
export default function StyleMirror() {
  ...
  const generate = useCallback(async () => {
    if (wordCount(seed) < 30) { setError("Please write at least 3 meaningful sentences (30+ words)."); return; }
  ...
  return (
    <div style={{ fontFamily:FONTS.ui, maxWidth:800, margin:"0 auto", padding:"2rem 1rem" }}>
      ...
      <div style={{ marginBottom:"1.5rem" }}>
        <div style={{ display:"flex", alignItems:"baseline", gap:10, flexWrap:"wrap" }}>
          ...
          <span style={{ marginLeft:"auto", fontSize:11, padding:"3px 8px", borderRadius:8,
            ...
            border:      "0.5px solid",
            borderColor: backendOk===null?"#D3D1C7":backendOk?"#97C459":"#F0997B",
            fontFamily:  FONTS.ui,
          }}>
            {backendOk===null?"checking…":backendOk?"backend ✓":"backend offline"}
          </span>
        </div>
      </div>
      ...
      <div style={{ display:"flex", borderBottom:"0.5px solid #D3D1C7", marginBottom:"1.5rem", gap:4, overflowX:"auto" }}>
        {visibleTabs.map(t => <Tab key={t.id} label={t.label} icon={t.icon} active={tab===t.id} onClick={() => setTab(t.id)}/>)}
      </div>
        ...
        <div className="animate-fadeIn">
          ...
          <div style={{ marginBottom:"1rem" }}>
            ...
            <textarea value={seed} onChange={e => setSeed(e.target.value)}
              placeholder="Write at least 3 meaningful sentences on your new topic…"
      ...
      {tab === "output" && result && (
        <div ref={outputRef} className="animate-fadeIn">
          ...
          <div style={{ marginTop:"1rem", padding:"12px 16px", background:"#EAF3DE", border:"0.5px solid #C0DD97", borderRadius:10, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <span style={{ fontSize:13, color:"#3B6D11", fontFamily:FONTS.ui }}>Add this piece to your style profile</span>
            <button onClick={() => { setSamples(prev => [...prev, { title:`Generated ${new Date().toLocaleDateString()}`, text:seed+"\n\n"+result.text }]); setTab("samples"); }}
              style={{ padding:"6px 14px", background:"#3B6D11", color:"#fff", border:"none", borderRadius:8, cursor:"pointer", fontFamily:FONTS.ui, fontSize:12, fontWeight:500, whiteSpace:"nowrap", marginLeft:12 }}>
              Add to Profile
            </button>
          </div>
        </div>
